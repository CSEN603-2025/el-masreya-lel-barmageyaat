...
const AppointmentScheduler = ({
  ...
  const [userType, setUserType] = useState(
    context === "report_review" ? "scad" : "student"
  );
  ...
  useEffect(() => {
    const savedAppointments = localStorage.getItem("appointments");
    const existingAppointments = savedAppointments
    ...
    localStorage.setItem(
      "appointments",
      JSON.stringify([...otherAppointments, ...appointments])
    );
    ...
    setAppointments(
      appointments.map((app) => {
        if (app.id === appointmentId) {
          ...
          if (
            updatedApp.scadApproval === "rejected" ||
            updatedApp.studentApproval === "rejected"
          ) {
            updatedApp.status = "rejected";
          } else if (
            updatedApp.scadApproval === "approved" &&
            updatedApp.studentApproval === "approved"
          ) {
  ...
  const getApprovalButtons = (appointment) => {
    const approvalType =
      userType === "scad" ? "scadApproval" : "studentApproval";
    ...
    if (currentApproval === "approved") {
      return (
        <button
          onClick={() =>
            handleApproval(appointment.id, approvalType, "rejected")
          }
      ...
      return (
        <button
          onClick={() =>
            handleApproval(appointment.id, approvalType, "approved")
          }
      ...
      <div className="mb-6">
        ...
        {showUserTypeToggle && (
          <div className="flex gap-4 mb-6">
            ...
            <button
              className={`px-4 py-2 rounded ${
                userType === "scad" ? "bg-blue-500 text-white" : "bg-gray-200"
              }`}
            ...
            {context !== "report_review" && (
              <div>
                ...
                >
                  ...
                  <option value="report clarification">
            ...
            <tbody>
              {appointments.map((appointment) => (
                <tr key={appointment.id} className="border-t">
                  ...
                  {context !== "report_review" && (
                    <td className="px-4 py-2">{appointment.purpose}</td>
                  )}
                  {userType === "scad" && (
                  ...
                  <td className="px-4 py-2">
                    <span
                      className={`px-2 py-1 rounded text-sm ${
                        appointment.scadApproval === "approved"
                          ? "bg-green-100 text-green-800"
                          : appointment.scadApproval === "rejected"
                  ...
                  <td className="px-4 py-2">
                    <span
                      className={`px-2 py-1 rounded text-sm ${
                        appointment.studentApproval === "approved"
                          ? "bg-green-100 text-green-800"
                          : appointment.studentApproval === "rejected"
                          ? "bg-red-100 text-red-800"
                          : "bg-yellow-100 text-yellow-800"
                      }`}
                    >

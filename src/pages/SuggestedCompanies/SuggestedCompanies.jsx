...
function SuggestedCompanies({ currUser, companyUsers, studentUsers }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const interestBasedSuggestions = findCompaniesByInterests(currUser, companyUsers);
      
      // Get companies recommended by other students
      const peerRecommendations = findCompaniesByPeerRecommendations(currUser, companyUsers, studentUsers);
      
      // Get companies based on your major
      const majorBasedSuggestions = findCompaniesByMajor(currUser, companyUsers);
  ...
  const findCompaniesByInterests = (student, companies) => {
    ...
    return companies.filter(company => {
      ...
      if (company.industry && interests.includes(company.industry.toLowerCase())) {
        return true;
      }
      ...
      if (company.internships) {
        return company.internships.some(internship => {
          const description = internship.description ? internship.description.toLowerCase() : '';
    ...
    students.forEach(otherStudent => {
      ...
      if (otherStudent.appliedInternships) {
        otherStudent.appliedInternships.forEach(application => {
          const company = companies.find(c => c.username === application.companyUsername);
          if (company) {
  ...
  const getMajorKeywords = (major) => {
    ...
    const majorKeywordsMap = {
      'Computer Science': ['software', 'programming', 'algorithm', 'data structure', 'computer', 'code', 'database', 'development'],
      'Information Technology': ['IT', 'network', 'system', 'infrastructure', 'support', 'helpdesk', 'security'],
      'Computer Engineering': ['hardware', 'circuit', 'embedded', 'processor', 'architecture', 'system'],
      'Software Engineering': ['software', 'development', 'agile', 'scrum', 'testing', 'quality'],
      'Data Science': ['data', 'analytics', 'machine learning', 'statistics', 'visualization', 'prediction'],
      'Cybersecurity': ['security', 'encryption', 'firewall', 'threat', 'vulnerability', 'protection'],
      'Business Administration': ['management', 'administration', 'strategy', 'organization', 'leadership'],
      'Marketing': ['marketing', 'advertising', 'promotion', 'sales', 'brand', 'market research'],
      'Finance': ['finance', 'accounting', 'investment', 'budget', 'financial', 'banking'],
      'Engineering': ['engineering', 'design', 'analysis', 'system', 'technical', 'development'],
    ...
    return [...keywords, ...generalKeywords].map(keyword => keyword.toLowerCase());
  };
  ...
  const handleFilterChange = (filter) => {
    ...
    if (filter === 'all') {
      ...
      setFilteredCompanies(suggestedCompanies.filter(company => company.matchType === filter));
    }
  };
    ...
    <div className="suggested-companies-container">
      ...
      <div className="page-header">
        ...
        <p className="user-info">
          ...
          {currUser.major && <span> Major: <strong>{currUser.major}</strong></span>}
          {currUser.interests && currUser.interests.length > 0 && 
            <span> Interests: <strong>{currUser.interests.join(', ')}</strong></span>
          }
        </p>
      </div>
      ...
      <div className="filter-controls">
        ...
        <button 
          className={`filter-button ${filterType === 'interest' ? 'active' : ''}`} 
        ...
        <button 
          className={`filter-button ${filterType === 'recommendation' ? 'active' : ''}`} 
        ...
        <>
          {filteredCompanies.length > 0 ? (
              ...
              {filteredCompanies.map((company, index) => (
                <div key={index} className="company-card">
                    ...
                    {company.recommendationScore && (
                      <div className="recommendation-score">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <span key={i} className={i < Math.floor(company.recommendationScore) ? 'star filled' : 'star'}>★</span>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {company.recommendedBy && (
                    <p className="recommended-by">Recommended by: {company.recommendedBy}</p>
                  )}
                  ...
                  {company.internships && company.internships.length > 0 && (
                    <div className="internship-list">
                      ...
                      <ul>
                        {company.internships.slice(0, 2).map((internship, idx) => (
                          <li key={idx}>
                            <Link 
                              to={`/internshipDetails/${internship.internshipID}/${company.username}`}
                              className="internship-link"
                            >
                              {internship.title} - {internship.paid ? 'Paid' : 'Unpaid'}
                            </Link>
                          </li>
                        ))}
                      </ul>
                      {company.internships.length > 2 && (
                        <p className="more-internships">+{company.internships.length - 2} more internships</p>
                      )}
                    </div>
                  )}
            ...
            <div className="no-results">
              ...
              <p>Try adjusting your profile interests or major to get better recommendations.</p>
